...
begin
	...
	if not exists(select CodEscuela from TEscuela where CodEscuela=@CodEscuela)
		...
		begin
			select Mensaje = 'Se encontro el registro'		
...
begin
	...
	-- CodEscuela tiene que existir
		if exists(select a.CodEscuela from TAlumno a inner join TEscuela e 
		on a.CodEscuela = e.CodEscuela where e.CodEscuela = @CodEscuela)
...
EXEC spAgregarAlumno 'A06','Luciana','Martinez','Arequipa','2000-10-26','E03'
exec spListarAlumno
...
begin
	...
	if not exists(select CodAlumno from TAlumno where CodAlumno=@CodAlumno)
		begin
			select CodError = 3 , Mensaje = 'El alumno no existe'
		end
	else
		begin
			select Mensaje = 'Se encontro el registro'			
